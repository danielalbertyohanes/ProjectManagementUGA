...
@section('content')
    <form method="POST" action="{{ route('employee.store') }}">
        ...
        <input type="text" class="form-control @error('npk') is-invalid @enderror" id="dosenNpk" name="npk" required
            maxlength="6" pattern="\d{6}" inputmode="numeric" title="NPK must be exactly 6 digits and only numbers are allowed"
            value="{{ old('npk') }}" oninput="this.value = this.value.replace(/\D/g, '').slice(0,6)">
        @error('npk')
